...
function handleNfcFromIntentFilter() {
	...
	// This was historically done in cordova.addConstructor but broke with
	// PhoneGap-2.2.0.
	// We need to handle NFC from an Intent that launched the application, but
	// *after*
	// the code in the application's deviceready has run. After upgrading to
	// 2.2.0,
...
var ndef = {
	 ...
	 * @payload byte array, containing zero to (2 ** 32 - 1) bytes, must not be
	 *          null
	 * 
	 * @returns JSON representation of a NDEF record
	 * 
	 * @see Ndef.textRecord, Ndef.uriRecord and Ndef.mimeMediaRecord for
	 *      examples
	 */
	 ...
	 * @languageCode ISO/IANA language code. Examples: â€œfiâ€�, â€œen-USâ€�,
	 *               â€œfr- CAâ€�, â€œjpâ€�. (optional)
	 * @id byte[] (optional)
	 */
	textRecord : function(text, languageCode, id) {
		...
		// TODO need to handle UTF-16 see Text Record Type Definition Section
		// 3.2.1 Syntax, Table 3
	...
	smartPoster : function(ndefRecords, id) {
		...
		if (ndefRecords) {
			// make sure we have an array of something like NDEF records before
			// encoding
			if (ndefRecords[0] instanceof Object && ndefRecords[0].hasOwnProperty('tnf')) {
				payload = ndef.encodeMessage(ndefRecords);
			} else {
				// assume the caller has already encoded the NDEF records into a
				// byte array
				payload = ndefRecords;
			}
		} else {
	...
	encodeMessage : function(ndefRecords) {
		...
		var encoded = [], tnf_byte, type_length, payload_length, id_length, i, mb, me, // messageBegin,
		// messageEnd
	...
	decodeMessage : function(bytes) {
		...
		ndef_message = [], tnf_byte, header, type_length = 0, payload_length = 0, id_length = 0, record_type = [], id = [], payload = [];
		...
		while (bytes.length) {
			...
			if (header.sr) {
				...
				payload_length = ((0xFF & bytes.shift()) << 24) | ((0xFF & bytes.shift()) << 26) | ((0xFF & bytes.shift()) << 8)
						| (0xFF & bytes.shift());
			}
	 ...
	 * 
	 * See NFC Data Exchange Format (NDEF) Specification Section 3.2
	 * RecordLayout
	 */
...
var tagid = {
	id : function(data, perteConnaissace, freqRespi, sat, freqCard, pArt, win, fail) {
	...
	read_bilan_complementaire : function() {
		var x;
		cordova.exec(function(success) {
			// navigator.notification.alert(success, null, 'Success : ', 'Ok');
			x = success;
	...
	write_bilan_complementaire : function(data, oxygene, chocs, immobilisation, extension, maladies, hospitalisations, traitements, allergies, win,
			fail) {
		cordova.exec(function(success) {
			navigator.notification.alert(success, null, 'Success : ', 'Ok');
		}, function(error) {
	...
	write_destination : function(evacuation, destination, ville, code_postal, win, fail) {
		cordova.exec(function(success) {
			navigator.notification.alert("Transmis à la puce", null, 'Success : ', 'Ok');
		}, function(error) {
			navigator.notification.alert(error, null, 'Error : ', 'Ok');
	...
	write_champlibre : function(textfield, win, fail) {
		cordova.exec(function(success) {
			navigator.notification.alert("Transmis à la puce", null, 'Success : ', 'Ok');
		}, function(error) {
			navigator.notification.alert(error, null, 'Error : ', 'Ok');
	...
	write_lesion : function(lesion, win, fail) {
		cordova.exec(function(success) {
			navigator.notification.alert("Transmis à la puce", null, 'Success : ', 'Ok');
		}, function(error) {
			navigator.notification.alert(error, null, 'Error : ', 'Ok');
